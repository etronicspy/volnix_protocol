...
function App() {
  ...
  const handleMint = async (address: string, asset_type: string = "vlnx") => {
    await fetch('http://localhost:8000/api/god-mode/mint', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 snap-x">
                {state.blocks.length === 0 ? (
                  ...
                  state.blocks.slice().reverse().map(b => (
                    <div key={b.height} className="min-w-[200px] bg-gray-900 p-4 rounded border border-gray-700 flex flex-col gap-2 snap-start shrink-0 shadow-lg relative overflow-hidden">
        ...
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700 mb-8">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="bg-gray-900 px-4 py-2 rounded border border-gray-700">
              ...
              <span className="text-xl font-mono text-green-400">{state.market?.last_price.toFixed(2)} VLNX</span>
            </div>
          </div>
            ...
            <form onSubmit={handleCreateOrder} className="flex flex-wrap gap-4 items-end">
              <div className="flex flex-col gap-1">
                ...
                >
                  ...
                  {Object.values(state.accounts).filter(a => a.role !== 'none').map(acc => (
                    <option key={acc.address} value={acc.address}>{acc.address.substring(0, 12)}... ({acc.balance.toFixed(0)}V / {acc.shares?.toFixed(0)}S)</option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-400">Price (VLNX)</label>
                <input 
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-400">Amount (Shares)</label>
                <input 
                  type="number" step="0.1" min="0.1" required
                  value={orderAmount}
                  ...
                  {state.recent_txs.map((tx, idx) => (
                    <tr key={idx} className="border-b border-gray-700/50">
                      ...
                      <td className="py-2 text-green-400 font-mono">
                        {tx.amount ? `${tx.amount.toFixed(2)} VLNX` : '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-gray-700 text-gray-400">
                    ...
                    <th className="pb-3 font-medium">VLNX Balance</th>
                ...
                <tbody>
                  {Object.values(state.accounts).map((acc) => (
                    <tr key={acc.address} className="border-b border-gray-700/50">
                      ...
                      <td className="py-3 text-green-400">{acc.balance.toFixed(2)}</td>
                      <td className="py-3 text-orange-400">{acc.shares?.toFixed(2) || '0.00'}</td>
                      <td className="py-3">
                        <span className={`px-2 py-1 rounded text-xs uppercase tracking-wider ${
                          acc.role === 'none' ? 'bg-gray-700 text-gray-300' :
                      ...
                      <td className="py-3">
                        <div className="flex gap-2">
                          <button onClick={() => handleMint(acc.address, 'vlnx')} className="bg-gray-700 hover:bg-gray-600 px-2 py-1 rounded text-xs transition-colors">+1000 VLNX</button>
                          <button onClick={() => handleMint(acc.address, 'shares')} className="bg-gray-700 hover:bg-gray-600 px-2 py-1 rounded text-xs transition-colors">+100 Shares</button>
                          <select 
