...
Это выжимка из whitepaper (`docs/WHITEPAPER.md`, v4.21), посвящённая
...
- **EndBlock — завершение блока (ключевая фаза для WRT).** Здесь модули считают
  ...
  - **сжигают** `f_i = α·L_i` у **всех** declare и `b_i + s_i` у **прошедших** отбор;
  - считают **Fill = Σ f_i(все) + Σ(b_i+s_i)(прошедшие)** и держат **Fill ≤ L_total**;
  - **начисляют базовую эмиссию WRT** активным валидаторам (`b_i>0`)
    пропорционально доле LZN (Контур 1);
  - **делят комиссии блока** `F` по правилу `F·(b_i/B)` (**f_i** в дележ не входит);
  - применяют коридор сжигания `(1−λ)·L_total ≥ Σb_i ≥ λ·L_total` (λ ≤ 5/12), потолок Fill и потолок `K`, **отсекают низковесов**
