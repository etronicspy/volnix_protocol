...
**Version 4.21:** §5.4 burn corridor corrected to **(1 − λ) × L_total ≥ Σ b_i ≥ λ × L_total**, with **λ ≤ 5/12** (genesis λ = 1/3). **(v4.20)** §4.2 Type 1 **Guest** renamed **Citizen**; WRT-based governance uses **DAO voters** (distinct from wallet role **Citizen**). **(v4.19)** Genesis: two bootstrap wallets (Supplier + Validator, no ZKP), no Supervisor role; same MOA as for ZKP roles; genesis ANT = EpochBlocks × L_total
...
**Declaration and culling (CometBFT-compatible).** The validator sends **`MsgDeclareParticipation{b_i, s_i}`** in block **N**. The protocol computes entry-burn **f_i = α · L_i** (**DeclareEntryAlpha**, §7.2 item 13). **EndBlocker** of N collects declarations, applies constraints, the **Σ b_i corridor**, the **Fill ≤ L_total** cap, and signer cap **K**, and returns to CometBFT an **updated `ValidatorSet`** for block **N+1**—only validators that pass culling **sign** N+1. This **physically bounds** signer count (O(n²) mitigation). When the set is **successfully** applied (corridor and other conditions met), on N+1 burns are fixed: **every** declarer burns **f_i**; those who **passed** culling additionally burn **b_i + s_i**. Rewards and fees go to passed participants by **b_i**.

**Top-K by weight (K).** Among validators with valid declarations (**f_i + b_i + s_i ≤ L_i**) and other EndBlocker checks, **rank by descending** **w_i = s_i / L_i**. The active `ValidatorSet` for the next block includes **at most K** validators with the **largest** **w_i** (ties broken **deterministically**, e.g. by address/key). Genesis reference **K = 150**. **K** is **DAO**-tunable (§7.2 item 10) within code **min/max**. **λ**, the **Σ b_i** floor, the **Fill** cap, and top-K apply **coherently** (**one deterministic** step order—identical on all nodes).

Each **height**, each Validator declares **b_i** and **s_i**; the protocol adds **f_i**.

**(0) f_i — entry-burn (ANT).** **f_i = α · L_i**, where **α** (**DeclareEntryAlpha**) is a **DAO** parameter (§7.2 item 13): code bounds **1/100 ≤ α ≤ 1/10**; genesis reference **α = 1/50**. Entry grants **no** income rights and is **not** part of **B = Σ b_i** / fee split. On a successful block **f_i burns for every** `MsgDeclareParticipation` on that height (including those culled by λ / Fill / K).

**Joint cap: f_i + b_i + s_i ≤ L_i.** Entry plus income burn plus stake **cannot exceed** activated LZN (**L_i**, **1 LZN = 1 ANT**)—equivalently **b_i + s_i ≤ (1 − α) · L_i**. All three draw from one ANT pool.

**(1) b_i — ANT burn volume (income rights).** Range subject to the joint cap. The validator **chooses** **b_i**. If **b_i = 0**, **no** fee share **nor** base WRT (**§5.1**) for that height. On a successful block **b_i burns only for those who passed** culling.

**(2) s_i — participation stake (ANT).** ANT amount that **burns for those who passed** on a successful block and sets **consensus weight** in CometBFT `ValidatorSet`: **w_i = s_i / L_i**. Range **0 ≤ s_i ≤ L_i − f_i − b_i**. A **larger** fraction of LZN staked means **higher** consensus weight.

**Fee split.** Let **F** = total tx fees at that height, **B = Σ b_i** over **passed** participants. Validator **i**’s share of **F** is **F · (b_i / B)**. **f_i** is **not** in the split.

**Global burn cap (λ) and minimum threshold (Σ b_i only).** Corridor: **λ × L_total ≤ Σ b_i ≤ (1 − λ) × L_total**, summed over candidates after coherent culling, **L_total = Σ L_i**, **λ** (**BurnCapLambda**) set by **DAO** (§7.2 item 9; genesis **λ = 1/3**, **λ ≤ 5/12**). If proposed **Σ b_i** **exceeds** the upper bound, validators are **sorted by w_i ascending**; **lowest-weight** are **excluded** until **Σ b_i of the remainder ≤ (1 − λ) × L_total**. **Minimum:** **Σ b_i ≥ λ × L_total**; if unmet, the set **is not updated**—EndBlocker **retries** on the next height. **f_i** and **s_i** are **not** in the λ corridor.

**Fill and emission-contour cap.** Per height:

**Fill = Σ f_i** (over **all** declarers) **+ Σ (b_j + s_j)** (over those who **passed**).

Canon rule: **Fill ≤ L_total**. If after the λ corridor and other steps **Fill** exceeds **L_total**, EndBlocker **culls** lowest **w_i** until **Fill ≤ L_total** (coherent with top-K). The **Fill** cap aligns actual ANT destruction with the Supplier emission ceiling (**ANT_max_epoch = EpochBlocks × L_total**, §5.5): per block the network **cannot** destroy more than **L_total** ANT.

**Burns on a successful block.** Every declarer: **f_i**. Those who passed: also **b_i + s_i**. Culled validators: **b_i** and **s_i** are **not** burned (remain on wallet); only **f_i** is lost.

**Trade-off.** Entry **f_i** prices the application (anti-spam). **b_i** drives income rights; **s_i** drives consensus weight and cull order. Global **λ**, **Fill ≤ L_total**, and **K** create competition: overfill culls low **w_i**. **No** fee bonuses beyond **F · (b_i / B)**.
...
**Maximum ANT destroyed per epoch (planning ceiling).** Per height **Fill ≤ L_total** (§5.4). With **1 LZN = 1 ANT**, an **upper bound** on ANT destroyed in one epoch is:

**ANT_max_epoch = EpochBlocks × L_total**,

**EpochBlocks** in **blocks** (§7.2 item 11), **L_total = Σ L_i** aggregate activated LZN at the epoch boundary. This is **not** actual burn but a **deterministic ceiling** for planning Supplier ANT supply: it matches the per-block rule **Fill ≤ L_total** (**Fill** includes **f_i** of all declarers and **b_i + s_i** of those who passed).
...
* **PoVB & ANT structure:** per height—**f_i = α · L_i** (entry-burn for all declarers), **b_i** (burn, income rights) and **s_i** (stake / weight); **f_i + b_i + s_i ≤ L_i** (**1 LZN = 1 ANT**); on a successful block every declarer burns **f_i**, those who passed also **b_i + s_i**; **Fill = Σ f_i(all) + Σ(b_i+s_i)(passed)** and **Fill ≤ L_total**; λ corridor **Σ b_i only**; **w_i = s_i / L_i**; **b_i = 0 → zero** share; **α** DAO (**1/100 ≤ α ≤ 1/10**, genesis **1/50**); **ANT** only via **internal market**—**§4.1**, **§5**; validator **ANT perpetual**; **Supplier emission epoch**—**§5.5**; **MOA**—**§5.3**, **§3.3**.
...
9. **BurnCapLambda (λ)** — global burn cap and minimum per block: **Σ b_i ≤ (1 − λ) × L_total** (upper), **Σ b_i ≥ λ × L_total** (lower); if upper exceeded—weight-based culling; if lower missed—EndBlocker retries (§5.4). Genesis **λ = 1/3**; parameter **λ ≤ 5/12** (corridor width ≥ 1/6); **DAO** within code **min/max**.
...
13. **DeclareEntryAlpha (α)** — entry-burn coefficient **f_i = α · L_i** on participation declare (§5.4). Code bounds **1/100 ≤ α ≤ 1/10**; genesis **α = 1/50**. **α** is **not** in the λ corridor and grants **no** fee share; it counts toward **Fill**.

Items **6–7** do not change WRT/LZN tokenomics or replace **burn** economics; they set **operational** chain limits. Item **8** caps **Supplier** role capacity. Item **9** sets **burn intensity**. Item **10** caps **consensus signer load**. Items **11–12** anchor key **periods in blocks** (ANT epoch, WRT halving) to **WRT votes**. Item **13** sets the **declare entry** price (**α**). **Hard max supply ANT** is **not** introduced via DAO.
...
*End of whitepaper body (v4.21). For Russian see [WHITEPAPER_RU.md](./WHITEPAPER_RU.md). For detailed per-version change notes see [volnix_protocol.md](./volnix_protocol.md).*
