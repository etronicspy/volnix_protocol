...
- **Отвечает за:** объявление per-height `b_i`/`s_i`, entry **`f_i = α·L_i`**,
  расчёт веса `w_i = s_i / L_i`, формирование `ValidatorSet` для N+1 в EndBlocker,
  коридор `(1−λ)·L ≥ Σb_i ≥ λ·L` (λ ≤ 5/12), потолок **Fill ≤ L_total**, потолок K,
  сжигание `f_i` у всех declare и `b_i+s_i` у прошедших, дележ комиссий `F·(b_i/B)`,
  базовую награду WRT, слешинг/evidence.
- **Msg:** `DeclarePerHeightBurn`, `UpdateConsensusState`, `SetValidatorWeight`,
  `RegisterConsensusMapping`.
- **Хранит:** объявления участия, текущий вес валидаторов, агрегаты сжигания / Fill.
...
- **Связи:** меняет параметры всех остальных модулей (λ, K, α/DeclareEntryAlpha, EpochBlocks,
